...
int main(void) {
	...
	int yesorno; //게임을 불러올지말지.

	printf("기존의 게임을 불러오시겠습니까? 1.불러오기 2. 새로 시작.");
	scanf("%d", &yesorno);
	...
	*/
	...
	if (yesorno == 1) 
	{
		FILE*fp3 = (FILE*)fopen("mypoketmon_list.txt", "rt");
		fscanf(fp3, "%d", &position);


		for (int i = 0; i <= position; i++)
		{
			fscanf(fp3, "%s %s %d %d %d\n", &mylist[i].name, &mylist[i].type, &mylist[i].agress, &mylist[i].hphp, &mylist[i].hp);
		}
	...
	while (1)
	{
		...
		{
			int num;
			printf("게임을 저장하시겠습니까? 1. 저장 2. 그냥 끝내기\n\n");
			scanf("%d", &num);
			if (num == 1)
			{
				FILE*fp2 = (FILE*)fopen("mypoketmon_list.txt", "wt");
				fprintf(fp2, "%d\n", position);
				for (int i = 0; i <= position; i++)
				{
					//구조체 배열에 있는걸 배열로 옮기고 배열을 다시 프린팅해볼것.
